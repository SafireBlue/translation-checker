...
// tslint:disable-next-line:max-line-length
export async function QADiffFromSegments(segs: ISegment[], findWhat: FindWhat, options?: any): Promise<SegmentCheckResult[] | null> {
    const results: SegmentCheckResult[] = [];
    // tslint:disable-next-line:max-line-length
    await Promise.all(segs.map(async (seg) => await QADiffFromSegment(seg, findWhat, options).then((res) => res && results.push(res))));
    return (results.length === 0) ? null : results;
}

// tslint:disable-next-line:max-line-length
export async function QADiffFromLocFormat(lf: ILocFormat<ISegment>, findWhat: FindWhat, options?: any): Promise<LocFormatCheckResult | null> {
    const segmentCheckResults = await QADiffFromSegments(lf.Segments!, findWhat, options);
    // tslint:disable-next-line:max-line-length
    return segmentCheckResults ? new LocFormatCheckResult(segmentCheckResults![0].Name, lf, {segmentCheckResults}) : null;
}

// tslint:disable-next-line:max-line-length
export async function QADiffFromLocFormats(lfs: Array<ILocFormat<ISegment>>, findWhat: FindWhat, options?: any): Promise<LocFormatCheckResult[] | null> {
    const results: LocFormatCheckResult[] = [];
    // tslint:disable-next-line:max-line-length
    await Promise.all(lfs.map(async (lf) => await QADiffFromLocFormat(lf, findWhat, options).then((res) => res && results.push(res))));
    return results.length === 0 ? null : results;
}
